...
//Pages
  if ($_SESSION['lang'] == "ua") {
    ...
    $STH = $DBH->query('SELECT * from news_en');
  }
...
function show_news() {
  ...
  if ($_SESSION['lang'] == "ua") {
    ...
    $STH = $DBH->prepare("SELECT * FROM  comments_en WHERE  news_id =  :id ORDER BY  id ASC ");
  }
  $data = array('id' => $_GET['id']);
  $STH->execute($data);
  $count_records = $STH->rowCount();
...
function add_news() {
  ...
  if ((check_accses($_SESSION['rules'], 'add')) && isset($_SESSION['login'])) {
    if (isset($_POST['submit_add'])) {
      if ($_POST['title_eu'] !== '' && $_POST['text_eu'] !== '' && $_POST['title_ua'] !== '' && $_POST['text_ua'] !== '') {
        $STH = $DBH->prepare("INSERT INTO news_en SET title=:title, text=:text, author=:author, rating='0',date=:date");
      }
      $data = array(
    ...
    $html_main_content .= print_lg('Required field *', $_SESSION['lang']) . '<br><br>
      <form method="post" name="add">
      ' . print_lg('English version:', $_SESSION['lang']) . '<br>
      <b>' . print_lg('Title:', $_SESSION['lang']) . ' *</b><input type="text" name="title_en"><br>
      <b>' . print_lg('Text:', $_SESSION['lang']) . ' *</b><br><textarea name="text_en" cols="40" rows="5"> </textarea><br>
      ' . print_lg('Ukraine version:', $_SESSION['lang']) . '<br>
      <b>' . print_lg('Title:', $_SESSION['lang']) . ' *</b><input type="text" name="title_ua"><br>
      <b>' . print_lg('Text:', $_SESSION['lang']) . ' *</b><br><textarea name="text_ua" cols="40" rows="5"> </textarea><br>
...
function delete_news() {
  ...
  if ($_SESSION['login'] == $row['author'] || $_SESSION['rules'] == 'admin') {
    if ($_SESSION['lang'] == "ua") {
      ...
      $STH = $DBH->prepare("Delete FROM news_en WHERE id=:id;Delete FROM comments_en WHERE news_id=:id ");
    }
    $STH->execute($data);
    header("Location: /news/");
    exit;
  ...
  if (isset($_POST['submit_edit'])) {
    if ($_POST['title'] != '' && $_POST['text'] != '') {
      if ($_SESSION['lang'] == "ua") {
        ...
        $STH = $DBH->prepare("UPDATE news_en Set  title=:title, text=:text,date=:date where id=:id");
      }
...
// Add html
  $html_main_content .= '<b>' . print_lg('Add language', $_SESSION['lang']) . '</b><br/>
  <form method="post">
  <table><tr><td>' . print_lg('Text en', $_SESSION['lang']) . '</td>
  <td>' . print_lg('Text ua', $_SESSION['lang']) . '</td> </tr> <tr>
  <td><textarea rows="3" cols="35" name="text_en"></textarea></td>
  <td><textarea rows="3" cols="35" name="text_ua"></textarea></td>  </tr>
  <tr><td colspan=2><input type="submit" value="ok" name="add_lg"></td></tr>  </table>
  </form>';

//Search html
  $html_main_content .= '<b>' . print_lg('Search', $_SESSION['lang']) . '</b><br/>
  <form method="post"><input type="text" name="search" value="';
  $_POST['search'] ? $html_main_content .= $_POST['search'] : $html_main_content .= $_SESSION['search'];
  $html_main_content .= '"><input type="submit" value="ok" name="submit_search">
  <a href="/news/edit_language/clear">' . print_lg('Clear', $_SESSION['lang']) . '</a>
  </form><br>';

//Add controle
  if (isset($_POST['add_lg']) && (!empty($_POST['text_en']) || !empty($_POST['text_ua']))) {
    $STH = $DBH->prepare('Insert into lang SET text_en=:text_en, text_ua=:text_ua ');
    $STH->execute(array('text_en' => $_POST['text_en'], 'text_ua' => $_POST['text_ua']));
    $html_main_content .= print_lg('You information update sucsesful', $_SESSION['lang']) . '</br>';
  }
...
//Delete controle
  if ($_GET['id'] == 'delete') {
    ...
    $STH->execute($data);
    $html_main_content .= print_lg('You text delete sucsesful', $_SESSION['lang']) . '</br>';
...
//    Pedjing
    ...
    while ($row = $STH->fetch(PDO::FETCH_ASSOC)) {
      $html_main_content .= '<form  method="post">
        <textarea rows="3" cols="35" name="text_en">' . $row['text_en'] . '</textarea>
        <textarea rows="3" cols="35" name="text_ua">' . $row['text_ua'] . '</textarea>
        <input type="hidden" name="id" value="' . $row['id'] . '">
        <input type="submit" name="change" value="ok">
        <a href="/news/edit_language/delete&lg_id=' . $row['id'] . '"><img src=/news/images/delete.gif></a>
        </form>';
    }
    if ($num_pages != 1) {
      $html_main_content .= '<p>';
      for ($page = 1; $page <= $num_pages; $page++) {
        if ($page == $current_page) {
          ...
          $html_main_content .= '<a href="/news/edit_language/' . $page . '">' . $page . '</a> &nbsp;';
        }
      }
      $html_main_content .= '</p>';
    }
